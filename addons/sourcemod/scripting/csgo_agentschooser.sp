...
Action Command_Main(int client, int args)
{
	...
	menu.SetTitle("==== [ AGENTS MENU ] ====\n ");
	menu.AddItem("", "--- Choose Agent ---", ITEMDRAW_DISABLED);
	
	menu.AddItem("ct", "Counter-Terrorist (CT) Agents");
	menu.AddItem("t", "Terrorist (T) Agents");
	
	menu.AddItem("", " ", ITEMDRAW_SPACER);
	menu.AddItem("reset_ct", "Reset CT Agent");
	menu.AddItem("reset_t", "Reset T Agent");
...
int SelectTeam(Menu menu, MenuAction action, int client, int selection) 
{
	switch(action)
	{
		case	MenuAction_Select:
		{
			...
			if (StrEqual(info, "reset_ct")) {
				g_ctAgent[client] = "";
				PrintToChat(client, " \x04[Agents]\x01 CT Agent has been reset to default.");
				Command_Main(client, 0);
				return;
			}
			else if (StrEqual(info, "reset_t")) {
				g_tAgent[client] = "";
				PrintToChat(client, " \x04[Agents]\x01 T Agent has been reset to default.");
				Command_Main(client, 0);
				return;
			}
...
void OpenAgentsMenu(int client)
{
	...
	if(g_iTeam[client] == CS_TEAM_CT)
		menu.SetTitle("==== [ CT AGENTS ] ====\n ");
	else
		menu.SetTitle("==== [ T AGENTS ] ====\n ");
		
	menu.AddItem("", "--- Category ---", ITEMDRAW_DISABLED);
...
{
	if(agent > 0 && agent <= MaxClients && client != agent && g_hTimer[agent] != null)	{
